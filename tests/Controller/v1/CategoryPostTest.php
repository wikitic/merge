...
class CategoryPostTest extends WebTestCase
{
    ...
    {
        $client = $this->client;
        $client->request('POST', '/v1/categories');
    ...
    public function provide_postCategories()
    {
        ...
        foreach ($category as $k => $v) {
            $aux = $category;
            unset($aux[$k]);
            yield ['/v1/categories',  Response::HTTP_BAD_REQUEST, json_encode($aux), 'Bad request'];
        }
        yield ['/v1/categories',  Response::HTTP_BAD_REQUEST, json_encode([]), 'Bad request'];
    }
    ...
    public function test_postCategories_OK()
    {
        ...
        $client = $this->createAuthenticatedClient();
        $client->request('POST', '/v1/categories', [], [], ['CONTENT_TYPE' => 'application/json'], json_encode($data));
